...
final class ClientTest extends PHPUnit_Framework_TestCase
{
    ...
    /** @var string */
    const POSTCODE_API_POSTCODE = '5041EB';

    /** @var int */
    const POSTCODE_API_NUMBER = 21;

    /** @var string */
    const POSTCODE_API_CITY = 'Tilburg';

    /** @var float */
    const POSTCODE_API_LONGITUDE = 5.07717893166;

    /** @var float */
    const POSTCODE_API_LATITUDE = 51.566414786;

    /** @var string */
    const POSTCODE_API_ADDRESS_ID = '0855200000061001';

    /** @var string  */
    const POSTCODE_API_VALID_URL = 'https://api.postcodeapi.nu/v2/addresses/?postcode=4904ZR&from%5Bpostcode%5D=4904ZR&from%5Bid%5D=0826200000012452&from%5Bnumber%5D=95';

    /**
     * @expectedException VS\PostcodeAPI\Exception\InvalidApiKeyException
     *
    ...
    {
        $client = $this->createClient(
            $this->loadMockResponse('successful_list_postcode_and_number')
        );

        $response = $client->getAddresses(self::POSTCODE_API_POSTCODE, self::POSTCODE_API_NUMBER);
    ...
    {
        $client = $this->createClient(
            $this->loadMockResponse('successful_detail')
        );

        $address = $client->getAddress(self::POSTCODE_API_ADDRESS_ID);
    ...
    /**
     * @expectedException VS\PostcodeAPI\Exception\ServerErrorException
     *
     * @throws CouldNotParseResponseException
     * @throws Exception
     * @throws InvalidApiKeyException
     ...
     * @expectedException VS\PostcodeAPI\Exception\InvalidUrlException
     *
     * @throws CouldNotParseResponseException
     * @throws Exception
     * @throws InvalidApiKeyException
    ...
    public function testExpectedNoExceptionsWhenValidNextLinkIsSupplied()
    {
        ...
        $client->request(self::POSTCODE_API_VALID_URL);
    }
    ...
    {
        static::assertSame(strtoupper($address->postcode), self::POSTCODE_API_POSTCODE, 'Incoming postcode did not match the expected postcode');
        static::assertSame((string)$address->number, (string)self::POSTCODE_API_NUMBER, 'Incoming number did not match the expected number');
        static::assertSame($address->city->label, self::POSTCODE_API_CITY, 'Incoming city did not match the expected city');
